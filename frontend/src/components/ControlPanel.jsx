        ...
        <div className="p-4 border-b border-indigo-100 bg-indigo-50">
          ...
          {customWaypoints.length >= 2 && (
            <div className="space-y-2 mb-3">
              ...
              <input type="text" value={customDroneId} onChange={e => setCustomDroneId(e.target.value)}
                ...
                className="w-full bg-white border border-indigo-200 rounded-xl px-3 py-2 text-sm font-mono text-[#1d1d1f] focus:outline-none focus:border-indigo-500 transition" />
              <button onClick={handleLaunchCustomRoute}
                className="w-full flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold py-2 rounded-xl transition">
                <CheckCheck size={13} />
          ...
          <div className="flex gap-2">
            <button onClick={onClearCustomRoute} disabled={customWaypoints.length === 0}
              className="flex-1 text-xs py-1.5 rounded-xl text-indigo-600 bg-white border border-indigo-200 hover:bg-indigo-50 disabled:opacity-40 transition">
              Clear
            </button>
            <button onClick={onTogglePlanningMode}
              className="flex-1 text-xs py-1.5 rounded-xl text-[#6e6e73] bg-white border border-black/8 hover:bg-[#f5f5f7] transition flex items-center justify-center gap-1">
              <X size={11} /> Cancel
            </button>
          </div>
        </div>
      ...
      {!drone && (
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="text-center">
            <p className="text-xs text-[#aeaeb2] mb-2">No drone selected</p>
            ...
            {videoPath ? (
              ...
              <div className="space-y-2">
                <button onClick={() => fileInputRef.current?.click()} disabled={uploadingVideo}
                  className="w-full flex items-center justify-center gap-2 border-2 border-dashed border-black/10 hover:border-[#E4007F]/40 rounded-xl py-2.5 text-xs text-[#6e6e73] hover:text-[#E4007F] transition disabled:opacity-50">
                  {uploadingVideo
                ...
                <input type="text" value={videoPath}
                  ...
                  className="w-full bg-[#f5f5f7] border border-black/8 rounded-xl px-3 py-2 text-xs font-mono text-[#1d1d1f] placeholder-[#c7c7cc] focus:outline-none focus:border-[#E4007F] transition" />
              </div>
            )}
          </div>
  ...
  return (
    <button onClick={onClick}
      className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-medium transition-all
        ${active ? 'bg-white text-[#E4007F] shadow-sm border border-black/6' : 'text-[#6e6e73] hover:text-[#1d1d1f]'}`}>
      {icon}{label}
    </button>
  );
}
